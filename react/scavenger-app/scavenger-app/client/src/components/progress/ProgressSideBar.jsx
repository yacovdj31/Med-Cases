...
function normalizeState(state, completedFlag) {
...
function ProgressContent() {
  const { t } = useTheme();
  const [boxes, setBoxes] = React.useState([]);       // active country boxes (fallback)
  ...
  // fallback box map (only active boxes)
  ...
  // Assigned in order — prefer populated box from progress; fallback to /boxes map
  const items = React.useMemo(() => {
    const statuses = progress?.statuses || [];
    return statuses
      .map((st, idx) => {
        const id = String(st.boxId?._id || st.boxId);
        const box = (st.boxId && st.boxId._id) ? st.boxId : boxMap.get(id);
        if (!box) return null;
